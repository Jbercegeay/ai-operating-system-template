...
Runs the **Four Cs Audit** on the current client AIOS project. Reads (never writes) the project's operating manual, memory, skills, agents, MCPs, decisions, and references. Scores each of the Four Cs out of 25. Surfaces strengths and the top 3 leverage-weighted gaps with concrete next-step commands.
...
**Operating manual:** `CLAUDE.md` and/or `AGENTS.md` (root), `CLAUDE.local.md` (gitignored).
**Memory:** `MEMORY.md` (root), `~/.claude/projects/<id>/memory/MEMORY.md`, or `memory/` folder.
**Skills:** `.claude/skills/*/SKILL.md` and/or `.agents/skills/*/SKILL.md` — count + frontmatter.
**Agents:** `.claude/agents/*.md` and/or `.agents/agents/*.md` — count + frontmatter.
...
|---|---|---|
| Operating manual exists and is substantive (>200 words) | 5 | Read CLAUDE.md or AGENTS.md, count words |
| Identity / role / voice captured | 5 | Manual mentions business/operator + role/mission, OR rules folder exists |
...
| Criterion | Points | How to detect |
|---|---|---|
| 3+ skills installed | 10 | Count `.claude/skills/*/SKILL.md` and `.agents/skills/*/SKILL.md` without double-penalizing mirrored skills |
...
| 1+ agent defined | 5 | Count `.claude/agents/*.md` or `.agents/agents/*.md` ≥ 1 |
...
- **Read-only by default.** Never modify CLAUDE.md, AGENTS.md, memory, skills, or any project files. Only optional write is the audit report.
