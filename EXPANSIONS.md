...
| Folder / file | Purpose |
|---|---|
| `context/` | About the operator, business, and priorities. Filled by `$onboard` or `/onboard`. |
...
| `connections.md` | Registry of every system the client AIOS can reach. |
| `.claude/skills/` | Claude Code skills: `/onboard`, `/audit`, `/level-up`. Add more via `/level-up`. |
| `.agents/skills/` | Codex skills: `$onboard`, `$audit`, `$level-up`. Keep aligned with `.claude/skills/` when operator mode is both. |
| `aios-intake.md` | Source-of-truth for onboarding. Edit and re-run any time. |
| `CLAUDE.md` | Claude Code operating manual. Filled by onboarding when using Claude Code or both. |
| `AGENTS.md` | Codex operating manual. Filled by onboarding when using Codex or both. |
...
| `.claude/agents/` or `.agents/agents/` | You need a sub-assistant for repeatable, multi-step research/writing | Agents run in their own context — keep the main session lean |
...
- `decisions/log.md` — every meaningful decision (`$level-up` or `/level-up` Phase 2 captures these automatically)
...
- `CLAUDE.md` / `AGENTS.md` — quarterly review; rewrite the persona/priorities section after the Q90 review
...
- **Don't fork your operating manuals.** Keep one root `CLAUDE.md` and one root `AGENTS.md` when running both modes. Sub-OS folders can have scoped manuals, but the root is canonical.
...
3. **Could `$level-up` or `/level-up` route a future skill into here naturally?** If yes, the AIOS will use it. If no, you're organizing for yourself, not for the system.
